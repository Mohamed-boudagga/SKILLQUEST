...
public class UIManager {
    ...
    private final Rectangle healthBarBg, healthBarFill;
    private final Label scoreLabel, speedLabel, slowLabel;
    private final VBox pauseOverlay, mainMenu, gameOverMenu, quizPanel;
    private final Label finalScoreLabel, quizQuestionLabel, quizResultLabel;
    private final TextField quizAnswerField;
    private final Button quizSubmitBtn;
    private Runnable onStart, onRestart, onQuizSubmit;
    ...
    public UIManager(Pane gamePane) {
        ...
        healthBarBg.setFill(Color.DARKRED); healthBarBg.setStroke(Color.WHITE); healthBarBg.setStrokeWidth(1);
        ...
        slowLabel.setTextFill(Color.CYAN); slowLabel.setVisible(false);
        ...
        quizPanel.setPrefSize(400, 250); quizPanel.setMaxSize(400, 250);
        quizPanel.setStyle("-fx-background-color: rgba(20,20,40,0.95); -fx-border-color: magenta; -fx-border-width: 3;");
        quizPanel.setLayoutX(200); quizPanel.setLayoutY(175);
    ...
    public void showMainMenu() {
        mainMenu.setVisible(true); hud.setVisible(false);
        pauseOverlay.setVisible(false); gameOverMenu.setVisible(false); quizPanel.setVisible(false);
    }

    public void showHUD() {
        mainMenu.setVisible(false); hud.setVisible(true);
        pauseOverlay.setVisible(false); gameOverMenu.setVisible(false);
    }

    public void showPause(boolean show) { pauseOverlay.setVisible(show); }

    public void showGameOver(int score) {
        hud.setVisible(true); gameOverMenu.setVisible(true);
        finalScoreLabel.setText("Final Score: " + score);
    }

    public void showQuiz(String question) {
        quizPanel.setVisible(true); quizQuestionLabel.setText(question);
        quizAnswerField.clear(); quizResultLabel.setText(""); quizAnswerField.requestFocus();
    }

    public void hideQuiz() { quizPanel.setVisible(false); }
    ...
    public String getQuizAnswer() { return quizAnswerField.getText(); }

    public void updateHUD(int health, int maxHealth, int score, double speedLevel, boolean slowActive) {
        healthBarFill.setWidth(200 * ((double) health / maxHealth));
